...
`dailySleepHistory`, and the health v2.10 session lists `workoutHistory` and
`sleepSessionHistory`: every `owl:ObjectProperty` with domain `health:HealthProfile` in this
pattern.

**"Latest reading" is the newest `*History` entry (health v2.10).** The convenience properties
`health:restingHeartRate`, `health:heartRateVariability`, `health:bodyMass` and their siblings are
declared `owl:DatatypeProperty` with `rdfs:range xsd:double`, and they mean exactly that: a
number. `health:bodyMass 91.2` says "body mass is 91.2 kg". Earlier revisions of this section
used the same names to point at a whole reading object (code, value, unit, date, device), which
gave one name two incompatible meanings that no shape could catch. That use is dropped. A
container MAY carry the scalar if a writer wants a quick value; anything more than the number
(when, from which device, from how many samples) is read from the newest entry of the matching
`*History` list, which is a named individual with its own IRI. A reader that needs "the latest
reading" takes the entry with the greatest `health:periodEnd` (or, for an unaggregated reading
such as blood pressure, the greatest timestamp).
...
- `cascade:date` -- The display date (or, for non-aggregated metrics like blood pressure, the exact
  timestamp, see §12.4) of the observation, in the zone the aggregate was cut in
...
- `cascade:statistic` (core v3.10) -- Which statistic of the samples the value is: `average`,
  `sum`, `minimum`, `maximum`, `median`, `standardDeviation`, `variance`, `count`, `baseline`,
  `lowerBound`, `upperBound`. A mean and a maximum over the same samples are two entries
- `health:timeZone` (health v2.10) -- The IANA zone the day was cut in: the Pod's
  `cascade:dayZone` at aggregation time (`pod-structure.md` Section 3.6), recorded so that a later
  change of home zone never re-mints history
- `health:device` (health v2.10) -- The `health:Device` that produced it, by IRI (Section 12.12)
- Metric-specific properties -- Values, counts, and quality indicators
- `prov:wasGeneratedBy` -- Inline provenance activity identifying the source (this activity
  stays a blank node: it has no identity independent of the reading that carries it, and it keeps
  its `prov:label` for readers that only want text)

`health.shapes.ttl` v1.8 reports a missing interval, statistic or cut zone on the three daily
aggregate classes at `sh:Warning` (`health:AggregateReadingShape`), so data written before
v2.10 keeps validating.
...
|---|---|---|---|
| Resting HR (latest) | `health:restingHeartRate` | `xsd:double` | OPTIONAL scalar convenience value. Everything else about the latest reading is its newest `restingHeartRateHistory` entry (Section 12.2) |
| Walking HR (latest) | `health:walkingHeartRate` | `xsd:double` | OPTIONAL scalar convenience value, as above |
...
| Date | `cascade:date` | `xsd:dateTime` | Display date of reading |
| Period Start | `health:periodStart` | `xsd:dateTime` | UTC start of the interval (health v2.10; Warning if absent) |
| Period End | `health:periodEnd` | `xsd:dateTime` | UTC end of the interval, exclusive (health v2.10; Warning if absent) |
| Statistic | `cascade:statistic` | `xsd:string` | Which statistic the value is (core v3.10; Warning if absent) |
| Time Zone | `health:timeZone` | `xsd:string` | IANA zone the day was cut in (health v2.10) |
| Device | `health:device` | (URI) | The `health:Device` that produced it (health v2.10) |
| Sample Count | `cascade:sampleCount` | `xsd:integer` | Number of samples aggregated |
| Provenance | `prov:wasGeneratedBy` | (Blank node) | Source activity |
...
# Each History entry is a named individual (D-WELLNESS-1). health:restingHeartRate is
# the optional scalar convenience value; the latest reading itself is the newest
# History entry (12.2). Each day is cut in America/Los_Angeles (the pod's
# cascade:dayZone), so 2026-01-20 local is 08:00Z to 08:00Z next day.
<#heart-rate> a health:HeartRateData ;
    health:restingHeartRate "65"^^xsd:double ;
...
<urn:uuid:7c1b1e2a-0000-5000-8000-000000000021> a health:DailyVitalReading ;
    ...
    cascade:date "2026-01-21T08:00:00Z"^^xsd:dateTime ;
    health:periodStart "2026-01-21T08:00:00Z"^^xsd:dateTime ;
    health:periodEnd "2026-01-22T08:00:00Z"^^xsd:dateTime ;
    health:timeZone "America/Los_Angeles" ;
    cascade:statistic "average" ;
...
**Where the values come from (health v2.10).** Active energy, exercise minutes and stand hours are imported from Apple's own `<ActivitySummary>` element, one per day, which Apple has already de-duplicated across sources. They are source-supplied daily aggregates: written with `cascade:statistic "sum"` and the summary's date as the period, and no source-priority logic applies to them. Steps are not in `ActivitySummary` and still come from aggregating step samples. Google Health supplies daily resting heart rate, HRV, SpO2, respiratory rate and VO2 max (written as `health:DailyVitalReading` entries), but no daily step, distance or energy total in either of its export formats.

#### 12.5.1 Properties Table -- DailyActivitySnapshot

| Property | Predicate URI | XSD Type | Description |
|---|---|---|---|
| Date | `cascade:date` | `xsd:dateTime` | Display date of activity |
...
# Activity data from Apple Watch. Each entry is a named individual (D-WELLNESS-1).
# Active energy, exercise minutes and stand hours come from Apple's ActivitySummary for
# the day; steps come from aggregating step samples. Every value on a snapshot is a sum.
...
Sleep data uses `health:SleepData` with two lists. `health:sleepSessionHistory` (health v2.10) holds `health:SleepSession` records, one per sleep episode per source per algorithm version (Section 12.6.3). `health:dailySleepHistory` holds `health:DailySleepSnapshot` records, the daily rollup derived from those sessions.

#### 12.6.1 Properties Table -- DailySleepSnapshot

| Property | Predicate URI | XSD Type | Description |
|---|---|---|---|
| Date | `cascade:date` | `xsd:dateTime` | Display date of the night (the date it ends) |
| Period Start / End | `health:periodStart` / `health:periodEnd` | `xsd:dateTime` | UTC interval (health v2.10) |
| Statistic | `cascade:statistic` | `xsd:string` | `"sum"` (core v3.10) |
| Duration | `health:durationHours` | `xsd:decimal` | Total sleep duration in hours |
| Derived from | `prov:wasDerivedFrom` | (URI) | The `health:SleepSession` record(s) the rollup was computed from |
| Sleep Quality | `health:sleepQuality` | (URI) | **DEPRECATED in health v2.10**; do not write. Reported at `sh:Warning` by `health:SleepQualitySpellingShape`. A source-supplied score is `health:sleepScore` on the session |
| Provenance | `prov:wasGeneratedBy` | (Blank node) | Source device activity |
...
# Sleep data from Apple Watch. Each entry is a named individual (D-WELLNESS-1). The
# daily snapshot is a rollup of the night's session(s), shown in 12.6.3.
<#sleep> a health:SleepData ;
    health:sleepSessionHistory (
        <urn:uuid:6b8f0c70-0000-5000-8000-000000000060>
    ) ;
...
<urn:uuid:5f7e9a50-0000-5000-8000-000000000050> a health:DailySleepSnapshot ;
    ...
    prov:wasDerivedFrom <urn:uuid:6b8f0c70-0000-5000-8000-000000000060> ;
    prov:wasGeneratedBy [ a prov:Activity ; cascade:sourceType "healthKit" ; prov:label "Apple Watch Series 9" ] .
...
#### 12.6.3 Sleep Sessions (health v2.10)

A `health:SleepSession` is one sleep episode as one source recorded it: the interval, the zone the source recorded, the algorithm that staged it, a minute total per stage, and, where the source supplies one, an overall score. Each stage property states its AASM mapping on itself:

| Property | Predicate URI | XSD Type | Source values | AASM |
|---|---|---|---|---|
| In bed | `health:inBedMinutes` | `xsd:decimal` | Apple `InBed` | **Not a stage.** Time available for sleep; overlaps the stage totals. Absent for Fitbit / Google, whose session interval is the time in bed |
| Awake | `health:awakeMinutes` | `xsd:decimal` | Apple `Awake`; Fitbit / Google `AWAKE` | W |
| Light | `health:lightSleepMinutes` | `xsd:decimal` | Apple `Core`; Fitbit / Google `LIGHT` | N1 + N2 |
| Deep | `health:deepSleepMinutes` | `xsd:decimal` | Apple `Deep`; Fitbit / Google `DEEP` | N3 |
| REM | `health:remSleepMinutes` | `xsd:decimal` | Apple `REM`; Fitbit / Google `REM` | R |
| Restless | `health:restlessMinutes` | `xsd:decimal` | Fitbit classic `RESTLESS` | **Not a stage.** A movement classification |
| Unspecified | `health:asleepUnspecifiedMinutes` | `xsd:decimal` | Apple `AsleepUnspecified`; Fitbit classic `ASLEEP` | Asleep, stage unknown |
| Score | `health:sleepScore` | `xsd:decimal` | Fitbit / Google `overall_score`, 0 to 100 | Source-supplied; Cascade never computes one |
| Algorithm | `health:algorithmVersion` | `xsd:string` | Google `sleep_session_v1` / `sleep_session_v2`; Apple `HKAlgorithmVersion` | |
| Zone | `health:timeZone` or `health:utcOffset` | `xsd:string` | Apple `HKTimeZone` (a name); Google `utc_offset` (an offset only) | |
| Source id | `health:sourceRecordId` | `xsd:string` | `"{space}:{id}"`, for example `google-health-sleep_id:...`; always a string | |
...
<urn:uuid:6b8f0c70-0000-5000-8000-000000000060> a health:SleepSession ;
    ...
    health:timeZone "America/Los_Angeles" ;
    health:inBedMinutes "462"^^xsd:decimal ;
    health:awakeMinutes "30"^^xsd:decimal ;
...
# zone name, the algorithm version, and the score the source supplied.
<urn:uuid:6b8f0c70-0000-5000-8000-000000000061> a health:SleepSession ;
    health:sourceRecordId "google-health-sleep_id:9187266553401923847" ;
    health:periodStart "2026-07-02T06:12:30Z"^^xsd:dateTime ;
    health:periodEnd "2026-07-02T13:40:00Z"^^xsd:dateTime ;
    health:utcOffset "-07:00" ;
    health:algorithmVersion "sleep_session_v2" ;
    health:awakeMinutes "41"^^xsd:decimal ;
    health:lightSleepMinutes "251"^^xsd:decimal ;
...
HRV data uses `health:HRVData` with an optional scalar latest value (`health:heartRateVariability`) and a history list (`health:hrvHistory`), whose newest entry is the latest reading (Section 12.2). The SDK also computes `health:HRVStatistics` for period-based summaries.
...
# HRV data with statistics and history. health:heartRateVariability is the optional
# scalar convenience value, a number as health.ttl declares it; the latest reading
# itself (when, from which device) is the newest health:hrvHistory entry, a named
# individual (12.2).
<#hrv> a health:HRVData ;
    health:heartRateVariability "42.5"^^xsd:double ;
    health:hrvHistory (
        <urn:uuid:8a1c2d30-0000-5000-8000-000000000080>
    ) ;
...
<urn:uuid:8a1c2d30-0000-5000-8000-000000000080> a health:HRVReading ;
    health:sdnn "42.5"^^xsd:double ;
    health:date "2026-02-18T07:00:00Z"^^xsd:dateTime ;
    health:device <urn:uuid:4d2e8f60-0000-5000-8000-000000000070> ;
    prov:wasGeneratedBy [ a prov:Activity ; cascade:sourceType "healthKit" ; prov:label "Apple Watch Series 9" ] .
```
...
# Body measurements container. health:bodyMass, health:bodyHeight, health:bodyMassIndex,
# health:bodyTemperature and health:oxygenSaturation are scalar convenience values, each a
# number as health.ttl declares it (owl:DatatypeProperty, range xsd:double). The reading
# behind the latest body mass, with its date, device and provenance, is the newest
# health:bodyMassHistory entry, a named individual (12.2).
<#body-measurements> a health:BodyMeasurements ;
    health:bodyMass "91.2"^^xsd:double ;
    health:bodyHeight "178"^^xsd:double ;
    health:bodyMassIndex "28.8"^^xsd:double ;
    health:bodyTemperature "36.6"^^xsd:double ;
    health:oxygenSaturation "97"^^xsd:double ;
    health:bodyMassHistory (
        <urn:uuid:2c4e6a90-0000-5000-8000-000000000090>
    ) .

# The day's body mass. A health:DailyVitalReading is a daily aggregate by definition, so
# it states its interval and statistic even when the day held a single weigh-in.
<urn:uuid:2c4e6a90-0000-5000-8000-000000000090> a health:DailyVitalReading ;
    ...
    cascade:date "2026-02-15T08:00:00Z"^^xsd:dateTime ;
    health:periodStart "2026-02-15T08:00:00Z"^^xsd:dateTime ;
    health:periodEnd "2026-02-16T08:00:00Z"^^xsd:dateTime ;
    health:timeZone "America/Los_Angeles" ;
    cascade:statistic "average" ;
    cascade:sampleCount "1"^^xsd:integer ;
...
**`health:AggregateReadingShape`** (shapes v1.8; targets `health:DailyVitalReading`, `health:DailyActivitySnapshot`, `health:DailySleepSnapshot`):

| Property | Constraint | Severity |
|---|---|---|
| `health:periodStart` | `sh:minCount 1`, `sh:maxCount 1`, `xsd:dateTime`; written as a UTC instant (`Z`) | Warning |
| `health:periodEnd` | `sh:minCount 1`, `sh:maxCount 1`, `xsd:dateTime`; written as a UTC instant (`Z`) | Warning |
...
**`health:WorkoutShape`** (shapes v1.8):

| Property | Constraint | Severity |
|---|---|---|
| `health:activityType` | `sh:minCount 1`, `sh:maxCount 1`, `xsd:string`, `{namespace}:{identifier}` | Violation |
| `health:periodStart`, `health:periodEnd` | `sh:minCount 1`, `sh:maxCount 1`, `xsd:dateTime` | Violation |
| `health:durationMinutes`, `health:distanceMeters`, `health:activeEnergyKcal` | `sh:maxCount 1`, `xsd:decimal`, `>= 0` | Violation |
| `health:averageHeartRate`, `health:maximumHeartRate` | `sh:maxCount 1`, `xsd:decimal`, 0-300 | Violation |
| `health:indoor` | `sh:maxCount 1`, `xsd:boolean` | Violation |
| `health:workoutRoute`, `health:device`, `health:snomedCode` | `sh:nodeKind sh:IRI` | Violation |

**`health:SleepSessionShape`** (shapes v1.8):

| Property | Constraint | Severity |
|---|---|---|
| `health:periodStart`, `health:periodEnd` | `sh:minCount 1`, `sh:maxCount 1`, `xsd:dateTime` | Violation |
| `health:algorithmVersion` | `sh:maxCount 1`, `xsd:string` | Violation |
| `health:sleepScore` | `sh:maxCount 1`, `xsd:decimal`, 0-100 | Violation |
| each `health:*Minutes` stage total | `sh:maxCount 1`, `xsd:decimal`, `>= 0` | Violation |
| `health:device` | `sh:nodeKind sh:IRI` | Violation |

**`health:SessionSoftShape`** (shapes v1.8; targets both session classes):

| Property | Constraint | Severity |
|---|---|---|
| `health:timeZone` | `sh:maxCount 1`, IANA zone name pattern | Warning |
| `health:utcOffset` | `sh:maxCount 1`, `+HH:MM` / `-HH:MM` | Warning |
| `health:sourceRecordId` | `sh:maxCount 1`, `xsd:string`, `{space}:{id}` | Warning |
| `health:periodStart`, `health:periodEnd` | written as a UTC instant (`Z`) | Warning |

**`health:DeviceShape`** (shapes v1.8):
...
The Violation constraints above apply only to classes new in health v2.10, which no earlier pod can contain.
    ...
    "@list": [
      {
        ...
        "cascade:date": { "@value": "2026-01-20T08:00:00Z", "@type": "xsd:dateTime" },
        "health:periodStart": { "@value": "2026-01-20T08:00:00Z", "@type": "xsd:dateTime" },
        "health:periodEnd": { "@value": "2026-01-21T08:00:00Z", "@type": "xsd:dateTime" },
        "health:timeZone": "America/Los_Angeles",
        "cascade:statistic": "sum",
        "health:steps": 7842,
...
### 12.12 Devices (health v2.10)

A device is a record with its own IRI, linked from each reading, aggregate or session by `health:device` (FHIR `Observation.device`; SOSA `madeBySensor`). `prov:wasGeneratedBy` is unchanged and keeps its label for readers that want only text.

**Identity** is minted once per Pod from the normalized `health:deviceName` plus `health:hardwareVersion` where one exists. Three things never take part in it, each for a measured reason: the raw device string Apple prints, whose embedded memory address (`<<HKDevice: 0x78a564f00>, ...`) changes on every export; the manufacturer, which Apple prints as `Apple` or `Apple Inc.` depending on software version; and the software version, which changes over a device's life. Google data has no per-sample hardware model and names one device two ways (`Google Fitbit Air` in data rows, `Fitbit Air` in `Devices.csv`), so a Google device is identified by its normalized name, with serial and firmware as attributes where the paired-device file has them.

| Property | Predicate URI | XSD Type | Identity | Description |
|---|---|---|---|---|
| Name | `health:deviceName` | `xsd:string` | Yes | Normalized name. REQUIRED |
| Hardware | `health:hardwareVersion` | `xsd:string` | Yes, where present | Apple `hardware:` (`Watch3,3`); Fitbit `hardware_rev` |
| Model | `health:deviceModel` | `xsd:string` | No | Apple `model:`; Fitbit `device_type` |
| Manufacturer | `health:deviceManufacturer` | `xsd:string` | **No** | Repeatable; each spelling seen |
| Software | `health:softwareVersion` | `xsd:string` | **No** | Repeatable; each version seen |
| Serial | `health:serialNumber` | `xsd:string` | **No** | Current paired device only (Fitbit) |

```turtle
@prefix cascade: <https://ns.cascadeprotocol.org/core/v1#> .
@prefix health:  <https://ns.cascadeprotocol.org/health/v1#> .

# Built from Apple's device string with the memory address discarded. Both
# manufacturer spellings seen across software versions are kept; neither is identity.
<urn:uuid:4d2e8f60-0000-5000-8000-000000000070> a health:Device ;
    health:deviceName "Apple Watch" ;
    health:deviceModel "Watch" ;
    health:hardwareVersion "Watch3,3" ;
    health:deviceManufacturer "Apple Inc." , "Apple" ;
    health:softwareVersion "6.1.1" , "6.2" ;
    cascade:dataProvenance cascade:DeviceGenerated .

# A Google Health device: normalized name ("Google " removed), no hardware model.
<urn:uuid:4d2e8f60-0000-5000-8000-000000000071> a health:Device ;
    health:deviceName "Fitbit Air" ;
    cascade:dataProvenance cascade:DeviceGenerated .
```

### 12.13 Workouts (health v2.10)

A `health:Workout` is one exercise session as the source recorded it, listed on the `health:ActivityData` container in `health:workoutHistory`.

| Property | Predicate URI | XSD Type | Cardinality | Description |
|---|---|---|---|---|
| Activity type | `health:activityType` | `xsd:string` | MUST (1) | The source's own identifier under a source namespace: `healthkit:HKWorkoutActivityTypeCycling`, `google-health:Outdoor Walk` (verbatim display string), `fitbit:90013` (legacy numeric `activityTypeId`) |
| SNOMED CT code | `health:snomedCode` | (URI) | MAY | Beside the source key, never instead of it |
| Period Start / End | `health:periodStart` / `health:periodEnd` | `xsd:dateTime` | MUST (1) each | UTC instants |
| Zone | `health:timeZone` or `health:utcOffset` | `xsd:string` | MAY | Apple `HKTimeZone`; Google offset |
| Duration | `health:durationMinutes` | `xsd:decimal` | MAY | As the source states it; may exclude pauses |
| Distance | `health:distanceMeters` | `xsd:decimal` | MAY | Converted to meters |
| Active energy | `health:activeEnergyKcal` | `xsd:decimal` | MAY | kcal |
| Average / maximum HR | `health:averageHeartRate` / `health:maximumHeartRate` | `xsd:decimal` | MAY | bpm, source-supplied |
| Indoor | `health:indoor` | `xsd:boolean` | MAY | Apple `HKIndoorWorkout`; absent is not false |
| Route | `health:workoutRoute` | (URI) | MAY | A `cascade:Attachment` holding the GPX file. Fitbit supplies none |
| Source id | `health:sourceRecordId` | `xsd:string` | MAY | `"{space}:{id}"`, always a string |
| Device | `health:device` | (URI) | MAY | Section 12.12 |

**Naming.** Where the source supplies an identifier (Apple `HKExternalUUID` or `HKMetadataKeySyncIdentifier`, Fitbit `logId`, Google `exercise_id`), D-CANONICAL-1 tier 1 names the workout from it, per id space; otherwise the digest tier applies. Google's ids exceed 2^53 and are carried as strings end to end.

**The route** is the GPX file itself, held as a content-addressed `cascade:Attachment` (core v3.7, `pod-structure.md` Section 4.3) and never parsed into triples. The property is `health:workoutRoute` because `health:route` is the route of administration of a vaccine dose.
...
<#activity> a health:ActivityData ;
    health:workoutHistory (
        <urn:uuid:1e3a5b70-0000-5000-8000-000000000100>
    ) .

<urn:uuid:1e3a5b70-0000-5000-8000-000000000100> a health:Workout ;
    health:activityType "healthkit:HKWorkoutActivityTypeCycling" ;
    health:sourceRecordId "healthkit-sync-identifier:6F1C0A2E-8B7D-4E4B-9D8A-3C2B1A0F9E8D" ;
    health:periodStart "2026-01-24T17:05:00Z"^^xsd:dateTime ;
    health:periodEnd "2026-01-24T18:12:30Z"^^xsd:dateTime ;
    health:timeZone "America/Los_Angeles" ;
    health:durationMinutes "64.5"^^xsd:decimal ;
    health:distanceMeters "28140"^^xsd:decimal ;
    health:activeEnergyKcal "712"^^xsd:decimal ;
    health:averageHeartRate "138"^^xsd:decimal ;
    health:maximumHeartRate "171"^^xsd:decimal ;
    health:indoor false ;
    health:workoutRoute <urn:uuid:1e3a5b70-0000-5000-8000-000000000101> ;
    health:device <urn:uuid:4d2e8f60-0000-5000-8000-000000000070> ;
    cascade:dataProvenance cascade:DeviceGenerated ;
    prov:wasGeneratedBy [ a prov:Activity ; cascade:sourceType "healthKit" ; prov:label "Apple Watch" ] .

# The route file, stored at its digest and described, never parsed.
<urn:uuid:1e3a5b70-0000-5000-8000-000000000101> a cascade:Attachment ;
    cascade:attachmentPath "attachments/sha-256/9b74c9897bac770ffc029102a200c5de8f1b3c6e5a4d2f0e9b8a7c6d5e4f3a2b" ;
    cascade:contentHash "9b74c9897bac770ffc029102a200c5de8f1b3c6e5a4d2f0e9b8a7c6d5e4f3a2b" ;
    cascade:hashAlgorithm "sha-256" ;
    cascade:attachmentMediaType "application/gpx+xml" ;
    cascade:attachmentTitle "route_2026-01-24_9.05am.gpx" .
```

A Fitbit workout has the same shape with `health:activityType "google-health:Outdoor Bike"` or `"fitbit:1071"`, an offset in `health:utcOffset`, its `exercise_id` or `logId` in `health:sourceRecordId`, and no `health:workoutRoute`: Fitbit exports GPS points without an exercise id, and a route cut from them by time window would be derived data, not what the source said.
